...
import { Room, User, GameType, Message } from './types.js';
...
const scribbleIntervals = new Map<string, NodeJS.Timeout>();

io.on('connection', (socket: Socket) => {
  console.log(`Socket connected: ${socket.id}`);

  // 1. CREATE ROOM
  socket.on('create-room', ({ nickname, userId }: { nickname: string; userId: string }) => {
  ...
  // 2. JOIN ROOM
  socket.on('join-room', ({ roomId, nickname, userId }: { roomId: string; nickname: string; userId: string }) => {
  ...
  // 3. THEME CHANGE
  socket.on('change-theme', (theme: 'space' | 'ocean' | 'arcade' | 'sakura' | 'carnival') => {
  ...
  socket.on('send-message', (msgData: { text?: string; imageUrl?: string; isOneTime: boolean }) => {
    const roomId = socket.data.roomId;
  ...
  socket.on('emoji-reaction', (emoji: string) => {
    const roomId = socket.data.roomId;
    if (roomId) {
  ...
  socket.on('select-game', (game: GameType) => {
    const roomId = socket.data.roomId;
    const room = rooms.get(roomId);
    if (!room) return;
    ...
    if (game === 'scribble') {
      ...
      const modifiers: ('opposite-hand' | 'circles-only' | 'one-line')[] = ['opposite-hand', 'circles-only', 'one-line'];
  ...
  socket.on('scribble-draw', (drawData: any) => {
    const roomId = socket.data.roomId;
    if (roomId) {
  ...
  socket.on('scribble-guess', (guess: string) => {
    const roomId = socket.data.roomId;
    const userId = socket.data.userId;
    const room = rooms.get(roomId);
    if (!room || !room.scribble) return;

    const session = room.scribble;
    if (session.drawerId === userId) return; // Drawer cannot guess
  ...
  socket.on('story-submit', (sentenceText: string) => {
    const roomId = socket.data.roomId;
    const userId = socket.data.userId;
    const room = rooms.get(roomId);
    ...
    if (session.turnUserId !== userId) return; // Not their turn
    ...
    let nextTwist: string | undefined = undefined;
    if (isTwistTurn) {
  ...
  socket.on('nhie-vote', (answer: 'have' | 'never') => {
    const roomId = socket.data.roomId;
    const userId = socket.data.userId;
    const room = rooms.get(roomId);
    if (!room || !room.nhie) return;
  ...
  socket.on('call-request', ({ type }: { type: 'voice' | 'video' }) => {
    const roomId = socket.data.roomId;
    const userId = socket.data.userId;
    if (roomId) {
  ...
  socket.on('call-response', ({ accepted }: { accepted: boolean }) => {
    const roomId = socket.data.roomId;
    if (roomId) {
